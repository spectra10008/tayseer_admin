@extends('layouts.app')
@section('title', 'الحالات الاجتماعية')
@section('content')
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0"> الحالات الاجتماعية</h2>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">الحالات الاجتماعية </h4>
                    ...
                    <div class="card-body card-dashboard">
                        ...
                        <div class="table-responsive">
                            <table class="table zero-configuration">
                                ...
                                <tbody>
                                    @foreach ($social_situations as $key => $social_situation)
                                        <tr>
                                            <td>{{ ++$key }}</td>
                                            <td>{{ $social_situation->situation_desc }}</td>
                                            <td>
                                                <button class="btn btn-success" name="edit_button"
                                                    value="{{ $social_situation->id }}" data-toggle="modal"
                                                    data-target="#edit_modal"><i class="fa fa-edit"></i></button>
                                                <button class="btn btn-danger mr-2"
                                                    onclick="if(confirm('هل أنت متأكد ؟')){document.getElementById('delete-users_{{ $social_situation->id }}').submit();}else{
                                            event.preventDefault();}"><i
                                                        class="fa fa-trash"></i></button>
                                                <form id="delete-users_{{ $social_situation->id }}"
                                                    action="/panel-admin/social-situations/{{ $social_situation->id }}" method="POST"
                                                    class="d-none">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
        ...
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="form form-vertical" action="/panel-admin/social-situations" method="POST"
                                ...
                                <div class="col-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control @error('situation_desc') is-invalid @enderror"
                                            name="situation_desc" placeholder="إسم الحالة" value="{{ old('situation_desc') }}"
                                            required>
                                    </div>
                                </div>
                                <div class="col-12">
    ...
    <script>
        $(document).ready(function() {
            $("button[name='edit_button']").click(function() {
                ...
                $.get("/panel-admin/social-situations/" + edit_val + "/edit", function(data, status) {
