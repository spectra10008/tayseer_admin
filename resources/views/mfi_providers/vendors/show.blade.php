                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-12 mt-3">
                            <a href="/panel-mfi/vendors/{{$vendor->id}}/edit" class="btn btn-primary mr-1"><i class="feather icon-edit-1"></i> تعديل</a>
                            <button class="btn btn-outline-danger" onclick="if(confirm('هل أنت متأكد ؟')){document.getElementById('delete-users_{{ $vendor->id }}').submit();}else{
                                ...
                                <form id="delete-users_{{ $vendor->id }}"
                                    action="/panel-mfi/vendors/{{ $vendor->id }}" method="POST"
                                    class="d-none">
                                    @csrf
                                    @method('DELETE')
                                </form>
